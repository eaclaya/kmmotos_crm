                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <div class="mt-2">
                    <p class="text-sm text-gray-500">
                      <div class="input-wrapper" v-if="clientVisible">
                        <p class="mt-2">Cliente</p>
                        <input type="text" class="px-4 py-2 w-full rounded-md border" v-model="clientValue" @input="searchClient" placeholder="Escribe nombre o telefono y presiona ENTER">
                        <ul  id="employee_list" class="shadow-xl w-full">
                          <li class="p-2 hover:bg-gray-100 cursor-pointer" v-for="client in clients"  @click="selectClient(client)">{{client.company_name ? client.company_name : client.name}} => {{client.phone}}</li>
                        </ul>
                      </div>  
  ...
  setup() {
    ...
    watchEffect(() => {
      if(store.state.task.id){
        clientVisible.value = store.state.task.client_id ? true : false
        employeeValue.value = store.state.task.employee_name
        clientValue.value = store.state.task.client_name
      ...
      if(selectedClient.value && selectedEmployee.value && description.value && employee){
        if(store.state.task.id){
          let task = {
            ...
            client_name: clientValue.value,
            employee_id: selectedEmployee.value.id,
            employee_name: employeeValue.value,
          ...
          for(let index in store.state.tasks){
            let currentTask = store.state.tasks[index]
            if(task.id == currentTask.id){
              store.state.tasks[index] = task
            }
          }
